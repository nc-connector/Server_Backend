...
class InstallSchema implements IRepairStep {
	...
	private function ensureSettingsTable(Schema $schema): bool {
		$tableName = $this->tableName('nccv_settings');
		...
		$table->addUniqueIndex(['config_key'], 'nccv_settings_key_uq');

		return true;
	}

	private function ensureSeatsTable(Schema $schema): bool {
		$tableName = $this->tableName('nccv_seats');
		if ($schema->hasTable($tableName)) {
			return false;
		}
		...
		$table->setPrimaryKey(['id']);
		$table->addUniqueIndex(['user_id'], 'nccv_seats_user_uq');

		return true;
	}

	private function ensureClientOverridesTable(Schema $schema): bool {
		$tableName = $this->tableName('nccv_client_overrides');
		if ($schema->hasTable($tableName)) {
			return false;
		}
		...
		$table->setPrimaryKey(['id']);
		$table->addUniqueIndex(['user_id', 'setting_key'], 'nccv_client_override_uq');
		$table->addIndex(['user_id'], 'nccv_client_override_user_idx');

		return true;
	}

	private function ensureGroupOverridesTable(Schema $schema): bool {
		$tableName = $this->tableName('nccv_group_overrides');
		if ($schema->hasTable($tableName)) {
			return false;
		}
		...
		$table->setPrimaryKey(['id']);
		$table->addUniqueIndex(['group_id', 'setting_key'], 'nccv_group_override_uq');
		$table->addIndex(['group_id'], 'nccv_group_override_group_idx');
