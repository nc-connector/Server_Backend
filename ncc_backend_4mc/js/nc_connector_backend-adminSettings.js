...
(() => {
	...
	function buildAppUrl(path, queryParams = null) {
		...
		const baseUrl = OC.generateUrl('/apps/nc_connector_backend' + normalizedPath)
		if (queryParams === null) {
			return baseUrl
		}
	...
	function normalizeImageSourceValue(src) {
		...
		if (
			...
			|| value.includes('/apps/nc_connector_backend/img/header.png')
	...
	async function main() {
		...
		refs.seatReportDownload.addEventListener('click', async () => {
			...
			try {
				...
				downloadTextFile(`nc_connector_backend-seat-report-${timestamp}.csv`, csv, 'text/csv;charset=utf-8')
