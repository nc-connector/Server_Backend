...
(() => {
	...
	const TEMPLATE_VARIABLES_BY_SETTING = {
		[SHARE_HTML_TEMPLATE_KEY]: ['URL', 'LINK_INTRO', 'LINK_LABEL', 'PASSWORD', 'EXPIRATIONDATE', 'RIGHTS', 'NOTE'],
	...
	const templateAssetRefreshModule = window.NCCBackendTemplateAssetRefresh
	if (!templateAssetRefreshModule) {
		console.error('nccb template asset refresh module missing')
		return
	}
	...
	const generalStatusUi = window.NCCBackendGeneralStatusUi
	if (!generalStatusUi) {
	...
	function getGeneralStatusUiHelpers() {
		return {
			escapeHtml,
			formatDate,
			formatDateTime,
	...
	async function main() {
		...
		templateEditor.setAssetRefreshHandler(templateAssetRefreshModule.createTemplateAssetRefreshHandler({
			api,
			extractExternalImageSourcesFromHtml: templateImages.extractExternalImageSourcesFromHtml,
			mergeSchema,
			readSettingControl,
			refs,
			root,
		...
		const renderProFunnel = (snapshot) => {
			generalStatusUi.renderProFunnel(refs, snapshot, getGeneralStatusUiHelpers())
		}

		const renderBackendUpdateStatus = (status) => {
			generalStatusUi.renderBackendUpdateStatus(refs, status, getGeneralStatusUiHelpers())
		}

		const renderRecommendedApps = (apps) => {
			generalStatusUi.renderRecommendedApps(refs, apps, getGeneralStatusUiHelpers())
		}
